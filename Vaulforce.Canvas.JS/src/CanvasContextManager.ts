...
export class ContextManager {
  ...
  public getActualRenderSize = (canvas: HTMLCanvasElement)=>{
    const dpi = window.devicePixelRatio || 1; // Get the device pixel ratio

    // Set canvas display size using CSS (handled by Tailwind w-full h-auto)
    const displayWidth = canvas.clientWidth;
    const displayHeight = canvas.clientHeight;

    // Set internal drawing buffer size to match display size * DPI for crisp rendering
    if (canvas.width !== displayWidth * dpi || canvas.height !== displayHeight * dpi) {
      return Array.from([displayWidth * dpi, displayHeight * dpi]);
    }
}
